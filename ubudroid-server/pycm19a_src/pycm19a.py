...
class ReceiveThread(threading.Thread):
    ...
    def on(self, house, unit):
        self.command(True, house, unit)

    def off(self, house, unit):
        self.command(False, house, unit)

    def command(self, on, house, unit):
        action = 'CMD_ON' if on else 'CMD_OFF'
        cmd = X10HACommand(action, house, unit)
        cmd.xmit(self.outEp)
...
def start_session():
	...
	#OutEp = usb.util.find_descriptor(cfg, bInterfaceNumber=interface_number, bAlternateSetting=alternate_setting)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            if len(line)>0:
                X10Send(line[0:len(line)-1], monitor.outEp)
            time.sleep(0.1)
    except:
        traceback.print_exc()
